...
window.switchTab = function (tab) {
    ...
    if (tab === 'personal') {
        ...
        formulaTab.classList.remove('active', 'border-blue-500', 'text-blue-600');
        ...
        personalTab.classList.remove('active', 'border-blue-500', 'text-blue-600');
    ...
    fetch(`/patients/${patientId}`)
        ...
        .then(data => {
            if (data.success && data.patient.optical_formula) {
                ...
                showMessage('Este paciente no tiene fórmula óptica registrada.', 'error');
            }
        })
        .catch(error => {
...
function displayFormula(patient) {
                ...
                <table class="min-w-full border border-gray-300">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.od?.esf || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.od?.cil || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.od?.eje || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.od?.add || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.od?.dpn || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.od?.alt || '-'}</td>
                        </tr>
                        <tr>
                            <td class="px-3 py-2 border border-gray-300 font-medium">OI</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.oi?.esf || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.oi?.cil || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.oi?.eje || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.oi?.add || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.oi?.dpn || '-'}</td>
                            <td class="px-3 py-2 border border-gray-300">${formula.formula.oi?.alt || '-'}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    if (formula.options) {
        const activeOptions = Object.keys(formula.options).filter(key => formula.options[key]);
        if (activeOptions.length > 0) {
            html += `
                <div>
                    <h5 class="font-medium text-gray-700 mb-2">Opciones</h5>
                    <div class="flex flex-wrap gap-2">
                        ${activeOptions.map(opt => `<span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">${opt.toUpperCase().replace('_', ' ')}</span>`).join('')}
                    </div>
                </div>
            `;
        }
                ...
                <div>
                    ...
                    <table class="w-full border border-gray-300">
                        ...
                        <tbody>
                            <tr>
                                <td class="px-3 py-2 border border-gray-300 font-medium">OD</td>
                                <td class="px-3 py-2 border border-gray-300">${formula.vision_lejana.od?.av_sc || '-'}</td>
                                <td class="px-3 py-2 border border-gray-300">${formula.vision_lejana.od?.av_cc || '-'}</td>
                            </tr>
                            <tr>
                                <td class="px-3 py-2 border border-gray-300 font-medium">OI</td>
                                <td class="px-3 py-2 border border-gray-300">${formula.vision_lejana.oi?.av_sc || '-'}</td>
                                <td class="px-3 py-2 border border-gray-300">${formula.vision_lejana.oi?.av_cc || '-'}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div>
                    ...
                    <table class="w-full border border-gray-300">
                        ...
                        <tbody>
                            <tr>
                                <td class="px-3 py-2 border border-gray-300 font-medium">OD</td>
                                <td class="px-3 py-2 border border-gray-300">${formula.vision_cercana.od?.av_sc || '-'}</td>
                                <td class="px-3 py-2 border border-gray-300">${formula.vision_cercana.od?.av_cc || '-'}</td>
                            </tr>
                            <tr>
                                <td class="px-3 py-2 border border-gray-300 font-medium">OI</td>
                                <td class="px-3 py-2 border border-gray-300">${formula.vision_cercana.oi?.av_sc || '-'}</td>
                                <td class="px-3 py-2 border border-gray-300">${formula.vision_cercana.oi?.av_cc || '-'}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
function fillFormulaFields(formula, prefix) {
    ...
    if (formula.formula) {
        ['od', 'oi'].forEach(eye => {
            ['esf', 'cil', 'eje', 'add', 'dpn', 'alt'].forEach(field => {
                const input = form.querySelector(`input[name="optical_formula[formula][${eye}][${field}]"]`);
                if (input && formula.formula[eye] && formula.formula[eye][field]) {
    ...
    if (formula.options) {
        Object.keys(formula.options).forEach(option => {
            const checkbox = form.querySelector(`input[name="optical_formula[options][${option}]"]`);
            if (checkbox) {
        ...
        if (formula[visionType]) {
            ['od', 'oi'].forEach(eye => {
                ['av_sc', 'av_cc'].forEach(field => {
                    const input = form.querySelector(`input[name="optical_formula[${visionType}][${eye}][${field}]"]`);
                    if (input && formula[visionType][eye] && formula[visionType][eye][field]) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.errors) {
                            ...
                            showMessage(data.message || 'Error al crear el paciente.', 'error');
                        }
                    }
                })
                .catch(error => {
            ...
            fetch(`/patients/${patientId}`, {
                ...
                .then(data => {
                    if (data.success) {
                        showMessage(data.message, 'success');
                        closeEditModal();
                        // Reload page to show updated patient
                        setTimeout(() => window.location.reload(), 1000);
                    } else {
                        if (data.errors) {
                            showFormErrors(data.errors, 'edit');
                        } else {
                            showMessage(data.message || 'Error al actualizar el paciente.', 'error');
                        }
                    }
                })
                .catch(error => {
